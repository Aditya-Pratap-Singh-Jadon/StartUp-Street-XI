...
const buildings = [
  ...
  { x: 19, w: 11, h: 54, start: 0.5, depth: "mid" },
...
function Building({ building, progress }) {
  const buildProgress = Math.max(
    0,
    Math.min(1, (progress - building.start) / 0.22),
  );
  ...
  const currentHeight = Math.max(0.015, buildProgress) * building.h;

  const floorsBuilt = Math.floor(buildProgress * windowRows);

  const opacity =
    building.depth === "far" ? 0.45 : building.depth === "mid" ? 0.65 : 0.9;
  ...
  return (
    <div
      ...
      style={{
        ...
        opacity: buildProgress > 0 ? opacity : 0,
      }}
    >
      {/* BUILDING SHADOW */}
      ...
      >
        ...
        {Array.from({
          ...
          const floorVisible = floor <= floorsBuilt;

          if (!floorVisible) return null;

          const bottom = 5 + floor * 5.2;
            ...
            >
              ...
              <div className="mt-2 grid grid-cols-3 gap-[14%] px-[10%]">
                {[0, 1, 2].map((windowIndex) => {
                  const lit =
                    (floor * 7 + windowIndex * 13 + Math.floor(building.x)) %
                      6 ===
                    0;
...
function ConstructionSite({ progress }) {
  ...
  const active = Math.max(0, Math.min(1, progress / 0.7));

  return (
    <div
      className="absolute bottom-[19%] left-0 right-0"
      style={{
        opacity: progress < 0.85 ? 1 : Math.max(0, 1 - (progress - 0.85) * 6),
      }}
    >
      ...
      >
        ...
        <div className="relative h-[28vh] w-[2px] bg-slate-300/25">
          ...
          <div className="absolute -top-1 left-[12vw] h-[13vh] w-px origin-top rotate-[8deg] bg-slate-300/20" />
...
function Street({ progress }) {
  const visible = Math.min(1, progress * 2.5);
...
export default function Background() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      targetProgress.current = Math.max(
        0,
        Math.min(1, 1 - window.scrollY / maxScroll),
      );
    };
  ...
  useEffect(() => {
    ...
    const animate = () => {
      setProgress((current) => {
        const next = current + (targetProgress.current - current) * 0.018;
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden bg-[#05080e]">
      ...
      {buildings.map((building, i) => (
        <Building key={i} building={building} progress={progress} />
      ))}
        ...
        {[18, 31, 44, 58, 72, 86].map((left, i) => (
          <div
            ...
            style={{
              ...
              bottom: `${5 + (i % 3) * 6}%`,
              background:
                i % 2 ? "rgba(244,126,104,.5)" : "rgba(100,190,210,.4)",
              boxShadow:
                i % 2
